...
export type KieConfig = {
  ...
  /**
   * True when serving as the hosted relay, which can never reach the caller's disk. Tools use it to
   * point the agent at KIE's own upload API instead of a local path. Unset for stdio.
   */
  remoteRelay?: boolean;
  docsDataDir?: string;
};
