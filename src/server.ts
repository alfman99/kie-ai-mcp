...
import { endpointMentionIndex, findMarketModel, marketModels, openapiEndpointCatalog, summarizeMarketModel, validateMarketInput } from "./registry.js";
...
function dataPathCandidates(fileName: string): string[] {
...
async function readDataFile(fileName: string): Promise<string> {
  const candidates = dataPathCandidates(fileName);
...
function addDocsResource(server: McpServer, name: string, uri: string, title: string, fileName: string, mimeType: string): void {
      ...
      contents: [
        {
          ...
          mimeType,
          text: await readDataFile(fileName)
        }
      ]
    })
  );
...
async function createMarketTask(args: {
  ...
  if (args.validateKnownModel ?? true) {
    validateMarketInput(args.model, args.input);
  }
...
export function createKieMcpServer(config: KieConfig = loadConfig(), fetchImpl?: typeof fetch): McpServer {
  ...
  addDocsResource(server, "kie-docs-analysis", "kie://docs/analysis", "KIE.AI Research Analysis", "ANALYSIS.md", "text/markdown");
  addDocsResource(server, "kie-openapi-catalog", "kie://docs/openapi-catalog", "KIE.AI OpenAPI Catalog", "openapi_endpoint_catalog.json", "application/json");
  addDocsResource(server, "kie-market-model-registry", "kie://docs/market-model-registry", "KIE.AI Market Model Registry", "market_model_registry.json", "application/json");
  addDocsResource(server, "kie-endpoint-index", "kie://docs/endpoint-index", "KIE.AI Endpoint Index", "endpoint_index.json", "application/json");
  ...
  server.registerTool(
    "kie_get_local_catalogs",
    {
      ...
      description: "Return bundled docs-derived catalog summaries without calling KIE.",
      inputSchema: {
